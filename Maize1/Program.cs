...
// 深さ優先探索(再帰なし) depth-first search (No Recursive)
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            for (; (str = Console.ReadLine()) != null;)
            {
                ...
                MazeSolverDFS solver = new MazeSolverDFS();
                Maze ans = solver.Solve(new Maze(col, row));
                sw.Stop();
