...
// 深さ優先探索(再帰なし) depth-first search (No Recursive) -- Stack
...
namespace Maze
{
    class Program
    {
        ...
        static void PrintResult(long sw, int goal, int bent)
        {
            Console.WriteLine("");
            Console.WriteLine("{0}ミリ秒", sw);
            Console.WriteLine("経路の数　　: " + goal);
            Console.WriteLine("最小の回数　: " + bent);
        }
                ...
                while (stack.Count != 0)
                {
                    ...
                    else
                    {
                        foreach (var pos in current.NextPositions())
                        {
                            ...
                            //next.Print();    // debug
                            if (next.cntBent < minBent) stack.Push(next);
                        }
                    }
                }
                return current;
